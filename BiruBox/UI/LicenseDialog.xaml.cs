...
public partial class LicenseDialog : Window
{
    ...
    private async void BtnActivate_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            if (result.IsGranted)
            {
                MessageBox.Show("License activated successfully! You may now use all BiruBox features.", "BiruBox Licensing", MessageBoxButton.OK, MessageBoxImage.Information);
                DialogResult = true;
                Close();
            }
            else
    ...
    private async void BtnClaimTrial_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            MessageBox.Show("Please enter your name.", "BiruBox Licensing", MessageBoxButton.OK, MessageBoxImage.Warning);
            return;
        }

        if (string.IsNullOrWhiteSpace(email) || !email.Contains('@'))
        {
            MessageBox.Show("Please enter a valid email address.", "BiruBox Licensing", MessageBoxButton.OK, MessageBoxImage.Warning);
            return;
        }
        ...
        try
        {
            var result = await LicensingManager.Client.ClaimTrialAsync(name, email, product: "BiruBox");
            LicensingManager.UpdateStatus(result);
            RefreshUiState();

            if (result.IsGranted)
            {
                MessageBox.Show("Your 14-day trial has started! All BiruBox features are now unlocked.", "Trial Activated", MessageBoxButton.OK, MessageBoxImage.Information);
                DialogResult = true;
                Close();
            }
            else
