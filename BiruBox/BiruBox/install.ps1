...
$loaderDll = Join-Path $here "BiruBox.Loader.dll"
...
foreach ($year in $Years) {
    ...
    if (Test-Path $loaderDll) { Copy-Item $loaderDll (Join-Path $binDest "BiruBox.Loader.dll") -Force }
...
Write-Host "`nInstallation completed successfully. Restart Revit. Ribbon: Birooni Tools > BiruBox & Modify > BiruBox"
