...
public static class LicensingManager
{
    ...
    public static async Task<LicenseValidationResult> InitializeAsync()
    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                var currentVersion = typeof(LicensingManager).Assembly.GetName().Version?.ToString(3) ?? "1.0.0";
                var update = await UpdateChecker.CheckAsync("BiruBox", currentVersion);
                if (update?.UpdateAvailable == true && !string.IsNullOrWhiteSpace(update.DownloadUrl))
                {
                    AvailableUpdate = update;
                    UpdateDetected?.Invoke(null, update);

                    // Seamless in-Revit update: Download and stage directly in background
                    var pluginDir = System.IO.Path.GetDirectoryName(typeof(LicensingManager).Assembly.Location);
                    if (!string.IsNullOrWhiteSpace(pluginDir))
                    {
                        var stagingDir = System.IO.Path.Combine(pluginDir, ".staging");
                        ...
                        {
                            IsUpdateStaged = true;
                            // Arm the exit watcher for current host process
                            AutoUpdateManager.ScheduleApplyOnExit(
    ...
    public static async Task<bool> DownloadAndInstallUpdateAsync(string downloadUrl)
    {
        try
        {
            ...
            var tempMsi = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "BiruBox-Setup.msi");
            await System.IO.File.WriteAllBytesAsync(tempMsi, bytes);
